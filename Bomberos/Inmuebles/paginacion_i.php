<?php 
	...
	if (isset($_GET["busqueda"])) {
		...
		$sql="SELECT * FROM inmueble WHERE rif LIKE :busqueda";
		$query=$bdd->prepare($sql);
		$query->execute(array(":busqueda"=>"%".$busqueda."%"));
		$bdd_filas=$query->rowCount();
		$pgn_total=ceil($bdd_filas/$filas);
		$query->closeCursor();
		$sql="SELECT * FROM inmueble WHERE rif LIKE :busqueda LIMIT $empezar, $filas";
		$query=$bdd->prepare($sql);
		$query->execute(array(":busqueda"=>"%".$busqueda."%"));
		...
		$pgn_total=ceil($bdd_filas/$filas);
		$query->closeCursor();
		$sql="SELECT * FROM inmueble LIMIT $empezar, $filas";
		$query=$bdd->query($sql);
		$inmueble_p=$query->fetchAll(PDO::FETCH_OBJ);
		$query->closeCursor();
	}
